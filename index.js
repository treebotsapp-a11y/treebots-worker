...
// Runs on any Node.js-capable cloud server (VPS / container).
// Polls the TreeBots control plane for assigned bots, starts/stops real
// Minecraft bots via mineflayer, and reports actual status + metrics back.
//
// Required env:
//   NODE_SECRET  — shared secret matching the app's NODE_SECRET setting
//   NODE_NAME    — the BotNode name this worker represents (e.g. "Cloud Node")
// Optional env:
//   API_BASE     — https://tree-bot-core.base44.app (default)
...
// botId -> { bot, startedAt, host, port, version, cfg }
...
function startBot(b) {
  ...
  // hold a ref even before spawn so we don't double-start
...
async function reconcile() {
  ...
  // 2) Reconcile desired running set
  ...
  // 3) Report metrics for running bots
